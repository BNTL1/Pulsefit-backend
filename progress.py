...
def ingest_sessions(user_id: str, planned_per_week: int, new_sessions: pd.DataFrame):
    """
    Optional: keep this for tests. This DOES NOT write to Firestore.
    """
    ...
    if user_id in SESS_STORE:
        SESS_STORE[user_id] = (
            pd.concat([SESS_STORE[user_id], new_sessions], ignore_index=True)
            .drop_duplicates()
        )
    else:
...
# ------------------- load from Firestore -------------------
def load_sessions_from_firestore(user_id: str) -> pd.DataFrame:
    """
    Read all sessions for a user from Firestore:
    ...
    for doc in docs:
        data = doc.to_dict() or {}
        date_val = data.get("date") or data.get("createdAt")
        effort = data.get("effort")

        # skip incomplete docs
        if date_val is None or effort is None:
            continue
        ...
        rows.append(
            {
                "date": date_val,
                "effort": str(effort).lower(),  # "hard", "medium", "easy"
            }
        )

    if not rows:
        raise ValueError(f"User {user_id} has no sessions in Firestore.")
...
def level_from_sessions(sessions_df: pd.DataFrame, planned_per_week: int = 4) -> pd.DataFrame:
    df = sessions_df.copy()
...
# ------------------- readiness & progress -------------------
def _ema(s: pd.Series, alpha: float = 0.25) -> pd.Series:
    return s.ewm(alpha=alpha, adjust=False).mean()


def build_readiness(wk_df: pd.DataFrame) -> pd.DataFrame:
    df = wk_df.copy()
...
def apply_progress_bar(df: pd.DataFrame, decision_period_weeks: int = 4) -> pd.DataFrame:
    df = df.copy()
    cur = df.iloc[0]["level_final"]
    prog = 50.0
    rank = {"beginner": 0, "intermediate": 1, "advanced": 2}
    ...
    for i, row in df.iterrows():
        delta = 0.6 * (row["Readiness"] - 65.0)
        prog = float(np.clip(prog + delta, -100, 100))

        is_decision = (i + 1) % decision_period_weeks == 0
        if is_decision:
            if prog >= 100 and cur != "advanced":
                cur = ["beginner", "intermediate", "advanced"][rank[cur] + 1]
                prog = 50.0
...
# ------------------- compute wrappers -------------------
def compute_progress_from_firestore(
    user_id: str,
    planned_per_week: int = 4,
    decision_period_weeks: int = 4,
) -> Tuple[dict, pd.DataFrame]:
    """
    Main entry point for API in production.
    Loads sessions from Firestore, then runs the same logic as before.
    """
    df = load_sessions_from_firestore(user_id)

    wk = level_from_sessions(df, planned_per_week=planned_per_week)
...
# Keep old compute_progress for tests with in-memory sessions
def compute_progress(user_id: str, decision_period_weeks: int = 4) -> Tuple[dict, pd.DataFrame]:
    if user_id not in SESS_STORE:
        raise ValueError(f"User {user_id} has no sessions in SESS_STORE.")
